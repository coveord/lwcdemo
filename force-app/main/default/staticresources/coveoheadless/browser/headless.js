(function (global, factory) {
	...
	(global = global || self, factory(global.CoveoHeadless = {}));
	...
	class t$1 extends Error{constructor(t){super("The following properties are invalid:"+t.map(t=>"\n"+t)),this.name="SchemaValidationError";}}class e{constructor(t){this.definition=t;}validate(e={}){const i={...this.default,...e},n=[];for(const t in this.definition){const e=this.definition[t].validate(i[t]);e&&n.push(`${t}: ${e}`);}if(n.length)throw new t$1(n);return i}get default(){const t={};for(const e in this.definition){const i=this.definition[e].default;void 0!==i&&(t[e]=i);}return t}}class i$1{constructor(t={}){this.baseConfig=t;}validate(t){return this.baseConfig.required&&r$1(t)?"value is required.":null}get default(){return this.baseConfig.default instanceof Function?this.baseConfig.default():this.baseConfig.default}required(){return !0===this.baseConfig.required}}function n$1(t){return void 0===t}function u$1(t){return null===t}function r$1(t){return n$1(t)||u$1(t)}class a$1{constructor(t={}){this.config=t,this.value=new i$1(t);}validate(t){const e=this.value.validate(t);return e||(l$1(t)?t<this.config.min?`minimum value of ${this.config.min} not respected.`:t>this.config.max?`maximum value of ${this.config.max} not respected.`:null:"value is not a number.")}get default(){return this.value.default}get required(){return this.value.required()}}function l$1(t){return n$1(t)||s$1(t)}function s$1(t){return "number"==typeof t}class o$1{constructor(t={}){this.value=new i$1(t);}validate(t){const e=this.value.validate(t);return e||(f$1(t)?null:"value is not a boolean.")}get default(){return this.value.default}get required(){return this.value.required()}}function f$1(t){return n$1(t)||c$1(t)}function c$1(t){return "boolean"==typeof t}const d$1={emptyAllowed:!0,url:!1},h$1=/^(?:(?:(?:https?|ftp):)?\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})).?)(?::\d{2,5})?(?:[/?#]\S*)?$/i;class v$1{constructor(t={}){this.config={...d$1,...t},this.value=new i$1(this.config);}validate(t){const e=this.value.validate(t);return e||(n$1(t)?null:g$1(t)?this.config.emptyAllowed||t.length?this.config.url&&!h$1.test(t)?"value is not a valid URL.":null:"value is an empty string.":"value is not a string.")}get default(){return this.value.default}get required(){return this.value.required()}}function g$1(t){return "[object String]"===Object.prototype.toString.call(t)}class p$1{constructor(t={}){this.config=t,this.value=new i$1(this.config);}validate(t){if(!r$1(t)&&!Array.isArray(t))return "value is not an array";const e=this.value.validate(t);if(null!==e)return e;if(r$1(t))return null;if(void 0!==this.config.max&&t.length>this.config.max)return "value contains more than "+this.config.max;if(void 0!==this.config.min&&t.length<this.config.min)return "value contains less than "+this.config.min;if(void 0!==this.config.each){let e="";return t.forEach(i=>{this.config.each.required&&r$1(i)&&(e="value is null or undefined: "+t.join(","));const n=this.validatePrimitiveValue(i,this.config.each);null!==n&&(e+=" "+n);}),""===e?null:e}return null}validatePrimitiveValue(t,e){return c$1(t)||g$1(t)||s$1(t)?e.validate(t):"value is not a primitive value"}get default(){}}

	const validatePayloadSchema = (payload, schemaDefinition) => {
	    const schema = new e(schemaDefinition);
	...
	const validatePayloadValue = (payload, schemaValue) => {
	    const schema = new e({ value: schemaValue });
	...
	const updateBasicConfiguration = createAction('configuration/updateBasicConfiguration', (payload) => validatePayloadSchema(payload, {
	    accessToken: new v$1({ required: true, emptyAllowed: false }),
	    organizationId: new v$1({ required: true, emptyAllowed: false }),
	}));
	/**
	...
	const updateSearchConfiguration = createAction('configuration/updateSearchConfiguration', (payload) => validatePayloadSchema(payload, {
	    searchApiBaseUrl: new v$1({ url: true, emptyAllowed: false }),
	}));
	/**
	...
	class PlatformClient {
	    static async call(options) {
	        const response = await browserPonyfill_1(options.url, {
	...
	***************************************************************************** */
	...
	const idDefinition = {
	    id: new v$1({ required: true, emptyAllowed: false }),
	};
	/**
	...
	const selectQuerySuggestion = createAction('querySuggest/selectSuggestion', (payload) => validatePayloadSchema(payload, {
	    ...idDefinition,
	    expression: new v$1({ required: true }),
	}));
	/**
	...
	const numberValue = new a$1({ required: true, min: 0 });
	/**
	...
	const criterionDefinition = {
	    expression: new v$1({ required: true, emptyAllowed: false }),
	};
	/**
	...
	const nonEmptyString = new v$1({ required: true, emptyAllowed: false });
	const nonEmptyArray = new p$1({
	    each: nonEmptyString,
	    required: true,
	});
	...
	const setPipeline = createAction('pipeline/set', (payload) => validatePayloadValue(payload, new v$1({ required: true, emptyAllowed: true })));
	...
	const optionsSchema = new e({
	     ...
	     * @default 5
	     */
	    numberOfSuggestions: new a$1({ default: 5, min: 0 }),
	    /**
	     ...
	     * @default false
	     */
	    isStandalone: new o$1({ default: false }),
	});
	/**
	...
	const optionsSchema$1 = new e({
	    /** The number of pages to display in the pager. */
	    numberOfPages: new a$1({ default: 5, min: 0 }),
	});
	...
	const schema = new e({
	     ...
	     * By default, a unique random identifier is generated.
	     */
	    facetId: new v$1({ default: () => randomID('facet') }),
	    /** The field whose values you want to display in the facet.*/
	    field: new v$1({ required: true }),
	    delimitingCharacter: new v$1({ default: '>' }),
	    filterFacetCount: new o$1({ default: true }),
	    injectionDepth: new a$1({ default: 1000 }),
	    numberOfValues: new a$1({ default: 8, min: 1 }),
	});
	...
	const prioritySchema = new e({
	    priority: new a$1({ required: false, default: 0, min: 0 }),
	});
	/**
	...
	exports.facetActions = facetSetActions;
	...
	exports.querySetActions = querySetActions;
	exports.querySuggestActions = querySuggestActions;
